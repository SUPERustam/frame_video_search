...
log_files=("$dir"/query_process_*.log)
metadata_files=("$dir"/query_metadata_*.csv)
hashes_dir=("$dir"/query_hashes/*)
...
if [ ${#log_files[@]} -gt 0 ]; then
    cat "${log_files[@]}" > "$dir/query_process.log"
else
    echo "No log files found to concatenate"
fi
sort -k1,2 -o "$dir/query_process.log" "$dir/query_process.log"
...
# If temp file is not empty, move it to final destination
if [ -s "$temp_file" ]; then
    mv "$temp_file" "$dir/query_metadata.csv"
    echo "Log files $dir/query_process_*.log successfully concatenated and sorted to $dir/query_process.txt"
    echo "Files $dir/query_metadata_*.csv successfully concatenated to $dir/query_metadata.csv"
